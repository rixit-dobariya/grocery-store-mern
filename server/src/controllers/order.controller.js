			...
			if (subtotal >= offer.minimumOrder) {
				...
				cart.items.forEach((item) => {
					...
					const productDiscount =
						(productTotal / totalBase) * discountAmount;
...
exports.getOrdersByUserId = async (req, res) => {
	...
	try {
		...
		// Step 3: Fetch order items for those orders
		...
		// Step 4 (Optional): Map items back to their orders
		const itemsGroupedByOrder = orderIds.map((orderId) => ({
			orderId,
		...
		res.status(200).json({ itemsGroupedByOrder });
	} catch (error) {
